...
| `delta-cli sandbox images` | 查看可用镜像列表（每条含 `image_name`/`tags`/`description`/`supports_shared_workspace`；`supports_shared_workspace=true` 表示该镜像支持共享工作区，可与 `--shared-workspace` 搭配） |
| `delta-cli sandbox recommend --cpu N --memory XGi` | 获取资源配置推荐 |
| `delta-cli sandbox create --image <img> [--no-auto-cleanup] [--shared-workspace=true\|false]` | 创建 sandbox 容器（`--no-auto-cleanup` 不被自动清理；`--shared-workspace` 默认 true：启用共享工作区，需要跨沙箱持久化时用 `supports_shared_workspace=true` 的镜像；false 跳过共享工作区播种/写回，适合快速任务） |
