...
| `delta-cli sandbox create --image <img> [--no-auto-cleanup] [--shared-workspace=true\|false]` | 创建 sandbox 容器（`--no-auto-cleanup` 不被自动清理；`--shared-workspace` 默认 true：启用共享工作区，需要镜像名含 juicefs；false 跳过共享工作区播种/写回，适合快速任务） |
