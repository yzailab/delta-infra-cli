...
3. **创建** `delta-cli sandbox create --image <image> --cpu 4 --memory 16Gi --gpu 1 --gpu-mem 8000 --max-life 120`（创建后立即可用，无需连接；--max-life 指定 sandbox 最大存活时间（分钟），默认 30。`--no-auto-cleanup` 可让 sandbox 不被自动清理，仅显式 kill/finish 可销毁。`--provider autodl` 可指定使用 AutoDL 后端。**`--gpu-mem`（单位 MiB）要匹配实际运行的模型**：模型参数量越大所需显存越大，大模型塞进过小显存会 OOM，宜先评估模型显存需求再设置。）
